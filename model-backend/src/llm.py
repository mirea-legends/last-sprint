...
class BaseLLM:
    def __init__(
        self, model_name: Optional[str] = None, model_path: Optional[str] = None
    ) -> None:
    ...
    def response(self, request: str) -> Any:
        return self.generate(
            self.chat_prompt_template.format(prompt=request), streaming=self.streaming
        )

    def memory_response(self, request: str, memory_queries: List[str]) -> Any:
        context = ""
        for i, query in enumerate(memory_queries):
            context += self.prompt_templates["memory_delimiter_ru"].format(
                i=i, query=query
            )

        queries = self.prompt_templates["memory_ru"].format(
            text=request, context=context
        )

        return self.generate(
            self.chat_prompt_template.format(prompt=queries), streaming=self.streaming
        )
...
class LlamaCPPLLM(BaseLLM):
    ...
    def generate(self, request: str, streaming: bool) -> Any:
        return self.gpt.create_completion(
            prompt=request, max_tokens=1024, stream=streaming
        )
...
class GigaChatLLM(BaseLLM):
    ...
    def generate(self, text, use_history=False):
        ...
        data = {
            ...
            "messages": (
                [] + self.message_history + [{"role": "user", "content": text}]
                if use_history
    ...
    def memory_response(self, request: str, memory_queries: List[str]) -> Any:
        ...
        request = self.prompt_templates["memory_ru"].format(
            text=request, context=context
        )
